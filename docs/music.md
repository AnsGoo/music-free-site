...
**音乐管理**是 Music Free 管理后台中的核心运维模块，路径为 **`/admin/music`**（侧栏「音乐管理」）。它将三类与曲库质量相关的能力整合在同一页面，通过 Tab 切换：
...
- **音乐使用**： 登录授权用户，，访问`/music`即可浏览所有音乐作品，并使用播放器播放。
...
当前系统不提供远程搜索和下载的插件，需要用户自行集成第三方插件
...
刮削成功后，宿主可将元数据**回写音频文件标签**，并更音乐数据库。
...
- 自动转码，当客户端的请求参数重要求的音频格式、码率与音乐文件本身的信息不符合时，会触发自动关转码，转成客户端需要的格式和码率流媒体。
